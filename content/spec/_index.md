...
* **Road/Surface**: `tarmac`, `gravel`, `sand`, `offtrack`, `fence`, `gate`
* **Services/Controls**: `fuel`, `service`, `speed_limit`, `neutralization`, `cp`, `wp`

### 5.4 Adventure Extensions

* `camp` — campsite
* `water` — potable water source
* `food` — restaurant or supply point
* `scenic` — viewpoint / landmark
* `border` — border crossing / checkpoint
* `town` — settlement / village

## 6. Validation

* A **JSON Schema** (or equivalent) must be published to validate OpenRoadBook files.
* Validation ensures:
  * `km` is numeric.
  * `cap` is 0–359.
  * `symbols` reference valid IDs.
  * Required fields are present.
* The draft adventure schema is available at `https://openroadbook.com/schemas/orb.schema.json` (JSON Schema Draft 2020-12). Versioned snapshots live under `https://openroadbook.com/schemas/1.0/orb.schema.json`, `https://openroadbook.com/schemas/1.1/orb.schema.json`, etc., so tools can pin to a specific release.

## 7. Interoperability

* Tools must support import/export of YAML and JSON.
* Converters may generate:
  * **PDF** (FIA‑style layout).
  * **GPX/KML** (for GPS devices).
  * **Digital roadbook formats** (RBK‑like).

## 8. Versioning

* Semantic versioning: **MAJOR.MINOR**.
* Breaking changes increment MAJOR.
* Additive changes (new fields, new symbols) increment MINOR.

## 9. Governance

* OpenRoadBook is community‑driven.
* Changes are proposed via public discussion (e.g., GitHub issues).
* A working group of organizers, developers, and competitors approves updates.
...
* Must use only FIA‑recognized symbols.
* Must comply with FIA formatting rules.
* Intended for competitive events.

### Adventure Profile

* May use extended symbols (Appendix B).
* May include optional fields like `tip:` or `landmark:` for descriptive guidance.
* Intended for non‑competitive navigation.
...
* **tip**: free text with practical advice (e.g., “Good place to refuel body and bike”).
* **landmark**: free text describing a visible landmark (e.g., “Look for the old stone bridge”).
